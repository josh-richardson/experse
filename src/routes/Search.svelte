<script>
	...
	$: searchTerm, postSearchResult = allPosts.filter(post => {
		return searchTerm != "" && (post.title.toLowerCase().includes(searchTerm.toLowerCase()) || post.body.toLowerCase().includes(searchTerm.toLowerCase()))
	})
</script>
